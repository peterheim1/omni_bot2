...
class CmdVelSubscriber(Node):
    """ROS2 Node that subscribes to the /cmd_vel topic and publishes wheel commands with acceleration control."""
    def __init__(self):
        ...
        # Initialize current and target velocities for each wheel
        self.current_velocities = {key: 0.0 for key in self.wheels.keys()}
        self.target_velocities = {key: 0.0 for key in self.wheels.keys()}

        # Create a publisher for wheel commands
        self.wheel_command_pub = self.create_publisher(String, 'wheel_commands', 10)

        # Set up a timer to update velocities periodically
    ...
    def cmd_vel_callback(self, msg):
        ...
        # Calculate target wheel data based on the latest cmd_vel
    ...
    def timer_callback(self):
        """Timer callback to update current velocities towards target velocities and publish commands."""
        with self.lock:
            for key in self.wheels.keys():
                target = self.target_velocities[key]
                current = self.current_velocities[key]
                delta_v = target - current

                # Determine acceleration direction
                if delta_v > 0:
                    # Accelerate
                    delta_v_step = min(delta_v, MAX_ACCELERATION * TIME_STEP)
                elif delta_v < 0:
                    # Decelerate
                    delta_v_step = max(delta_v, -MAX_DECELERATION * TIME_STEP)
                else:
                    delta_v_step = 0.0

                # Update current velocity
                self.current_velocities[key] += delta_v_step

            # After updating velocities, format and publish wheel commands
            self.publish_wheel_commands()

    def publish_wheel_commands(self):
        """Format the current velocities and publish to the 'wheel_commands' topic."""
        # Calculate wheel data based on current velocities
        wheel_data = self.calculate_wheel_data_from_current()
        ...
        # Assuming velocity scaling: m/s to cm/s by multiplying by 100
        # Adjust scaling as per Arduino's expected input
        V_F_L = int(round(wheel_data['S0']['velocity'] * 100))
        V_R_L = int(round(wheel_data['S1']['velocity'] * 100))
        V_R_R = int(round(wheel_data['S2']['velocity'] * 100))
        V_F_R = int(round(wheel_data['S3']['velocity'] * 100))

        # Ensure velocities are within Arduino's expected range
        V_F_L = max(min(V_F_L, 999), -999)
        V_R_L = max(min(V_R_L, 999), -999)
        V_R_R = max(min(V_R_R, 999), -999)
        V_F_R = max(min(V_F_R, 999), -999)

        # Create the message string
        message = 'a %d %d %d %d %d %d %d %d\r' % (
            S0_angle, S1_angle, S2_angle, S3_angle,
            V_F_L, V_R_L, V_R_R, V_F_R
        )
        self.get_logger().info(f"Wheel Command: {message.strip()}")  # Log without carriage return
    ...
    def calculate_wheel_data_from_current(self):
        """
        Calculate wheel angles and velocities based on current velocities.
        ...
        data = {}
        for key, (x_pos, y_pos) in self.wheels.items():
            # For simplicity, assume angles remain the same as target (could be enhanced)
            # Alternatively, store and update angles based on movement
            # Here, we'll just set angles to zero for demonstration
            angle = 0.0  # Placeholder; replace with actual angle calculation if needed

            # Get current velocity
            velocity = self.current_velocities[key]

            # Initialize reversal flag
            reversed_flag = False

            # Example condition: Reverse if velocity exceeds a threshold
            # Adjust as per your requirements
            # Here, we skip reversal logic for simplicity
